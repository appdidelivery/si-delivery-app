...
    const[cepError, setCepError] = useState('');
  ...
  useEffect(() => {
      ...
      const loadMarketingScripts = () => {
          ...
          // 2.5 Verificação de Domínio do Facebook (Meta)
          if (integrations?.meta?.domainVerification && !document.querySelector('meta[name="facebook-domain-verification"]')) {
              const meta = document.createElement('meta');
              meta.name = "facebook-domain-verification";
              // Remove espaços acidentais e pega só o código se o cliente colar a tag inteira por engano
              let cleanCode = integrations.meta.domainVerification.trim();
              if (cleanCode.includes('content="')) {
                  cleanCode = cleanCode.split('content="')[1].split('"')[0];
              }
              meta.content = cleanCode;
              document.head.appendChild(meta);
              console.log("🔵 Meta Tag de Verificação de Domínio inserida!");
          }
          ...
          <div className="space-y-3">
              <details className="group bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden [&_summary::-webkit-details-marker]:hidden">
                  ...
                  <div className="p-5 pt-0 text-sm text-slate-500 font-medium border-t border-slate-50 mt-1 leading-relaxed">
                      {(() => {
                          if (storeSettings.vacationMode?.active) {
                              return "No momento estamos em recesso/férias programadas. Retornaremos em breve!";
                          }

                          if (!storeSettings?.schedule) return "O nosso horário de atendimento varia de acordo com o dia.";
                          
                          const todayId = new Date().getDay();
                          
                          // Ajuste crucial: Tenta encontrar o dia de hoje no array dinâmico, se falhar, cai num fallback elegante.
                          const todayConfig = storeSettings.schedule[todayId];

                          if (!todayConfig || !todayConfig.open) {
                              return "Hoje estamos fechados para descanso. Verifique as nossas redes sociais ou simule um pedido para confirmar os horários de outros dias.";
                          }

                          // 🚨 CORREÇÃO: Garante que lê as horas do array (todayConfig.start) e não das variáveis globais antigas.
                          const safeStart = todayConfig.start || '08:00';
                          const safeEnd = todayConfig.end || '23:00';

                          let scheduleText = `Hoje atendemos das ${safeStart} às ${safeEnd}.`;
                          
                          if (todayConfig.splitShift && todayConfig.start2 && todayConfig.end2) {
                              scheduleText = `Hoje o nosso horário é dividido em dois turnos: das ${safeStart} às ${safeEnd} e depois retornamos das ${todayConfig.start2} às ${todayConfig.end2}.`;
                          }
                          return `${isStoreOpenNow ? 'Sim, estamos abertos agora! ' : 'No momento estamos fechados. '}${scheduleText}`;
                      })()}
                  </div>
              </details>
              ...
              <details className="group bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden [&_summary::-webkit-details-marker]:hidden">
                  ...
                  <div className="p-5 pt-0 text-sm text-slate-500 font-medium border-t border-slate-50 mt-1 leading-relaxed">
                      {(() => {
                          const pm = storeSettings.acceptedPayments || { online: true, pix: true, cardDelivery: true, cashDelivery: true, cardPickup: true, cashPickup: true };
                          const methods = [];
                          
                          if (pm.online || pm.pix) {
                              methods.push("online e seguro direto pelo aplicativo (Cartão de Crédito e Pix)");
                          }
                          
                          const deliveryMethods = [];
                          if (pm.cardDelivery) deliveryMethods.push("máquina de cartão");
                          if (pm.cashDelivery) deliveryMethods.push("dinheiro");
                          
                          if (deliveryMethods.length > 0) {
                              methods.push(`pagar na entrega via ${deliveryMethods.join(" ou ")}`);
                          }
                          
                          if (methods.length === 0) return "Aceitamos pagamentos conforme combinado no nosso atendimento.";
                          
                          return `Para a sua comodidade, aceitamos pagamento ${methods.join(" ou então, poderá ")}.`;
                      })()}
                  </div>
              </details>
              ...
              <details className="group bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden [&_summary::-webkit-details-marker]:hidden">
                  ...
                  <div className="p-5 pt-0 text-sm text-slate-500 font-medium border-t border-slate-50 mt-1 leading-relaxed">
                      {(() => {
                          const isDelivery = storeSettings.deliveryEnabled !== false;
                          const isPickup = storeSettings.pickupEnabled !== false;
                          
                          if (!isDelivery && isPickup) {
                              return "No momento não estamos a fazer entregas na residência. A nossa loja funciona exclusivamente com a opção de Retirada no Balcão.";
                          }
                          
                          let text = `Atendemos uma vasta região. É super simples: basta adicionar os produtos que deseja ao carrinho, introduzir o seu endereço de entrega e o nosso sistema irá calcular a viabilidade e a taxa de frete na hora.`;
                          
                          if (isPickup) {
                              text += " Caso prefira, disponibilizamos também a opção de retirar a sua encomenda no nosso balcão, totalmente sem custos de entrega.";
                          }
