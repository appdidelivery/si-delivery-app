...
import { 
  ...
  Save, X, MessageCircle, Crown, Flame, Trophy, Printer, Bell, PlusCircle, ExternalLink, LogOut, UploadCloud, Loader2, Image, List 
} from 'lucide-react';
...
export default function Admin() {
  ...
  const [categories, setCategories] = useState([]); // NOVA: Lista de categorias
  const [settings, setSettings] = useState({ promoActive: false, promoBannerUrl: '' });
  
  // Modais
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [form, setForm] = useState({ name: '', price: '', category: '', imageUrl: '', tag: '', stock: 0 }); // Com estoque
  const [editingId, setEditingId] = useState(null);

  const [isCatModalOpen, setIsCatModalOpen] = useState(false); // Modal Categoria
  ...
  const navItems = [
    ...
    { id: 'categories', name: 'Categorias', icon: <List size={18}/>, mobileIcon: <List size={22}/> }, // NOVA ABA
  ...
  const uploadToCloudinary = async (file, setLoad, setErr) => {
    if (!file) { setErr("Sem arquivo"); return null; }
    setLoad(true); setErr('');
    ...
    try {
      ...
      setLoad(false); return data.secure_url;
    } catch (e) { setErr(e.message); setLoad(false); return null; }
  };

  const handleProductImageUpload = async () => {
    const url = await uploadToCloudinary(imageFile, setUploading, setUploadError);
    if (url) { setForm(prev => ({ ...prev, imageUrl: url })); setImageFile(null); setUploadProgress(100); }
  };

  const handleLogoUpload = async () => {
    const url = await uploadToCloudinary(logoFile, setUploadingLogo, setLogoUploadError);
    if (url) { await updateDoc(doc(db, "settings", "store_status"), { storeLogoUrl: url }); setLogoFile(null); }
  };

  const handleBannerUpload = async () => {
    const url = await uploadToCloudinary(bannerFile, setUploadingBanner, setBannerUploadError);
    if (url) { await updateDoc(doc(db, "settings", "store_status"), { storeBannerUrl: url }); setBannerFile(null); }
  };

  const handlePromoBannerUpload = async () => {
    const url = await uploadToCloudinary(promoBannerFile, setUploadingPromoBanner, setPromoBannerUploadError);
    if (url) { await updateDoc(doc(db, "settings", "marketing"), { promoBannerUrl: url }); setPromoBannerFile(null); }
  };
  ...
  const printLabel = (o) => {
    ...
    w.document.write(`
      <html><body style="font-family:sans-serif;width:300px;padding:10px;">
        <center><h2>CONVENIÊNCIA SI</h2></center><hr>
            ...
            {products.filter(p => p.stock !== undefined && Number(p.stock) <= 2).length > 0 && (
                <div className="bg-red-50 border border-red-200 p-6 rounded-[2rem] animate-pulse">
                    ...
                    <div className="flex gap-2 flex-wrap mt-2">
                        {products.filter(p => p.stock !== undefined && Number(p.stock) <= 2).map(p => (
                            <span key={p.id} className="bg-white text-red-600 px-3 py-1 rounded-lg text-xs font-bold border border-red-100">
                                {p.name} ({p.stock} un)
                            </span>
                        ))}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                  ...
                  <p className="text-4xl font-black text-green-500 italic">
                    R$ {orders.filter(o => o.status === 'completed' && new Date(o.createdAt?.toDate()).toDateString() === new Date().toDateString())
              ...
              {products.map(p => (
                <div key={p.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 flex items-center gap-4 shadow-sm group hover:shadow-md transition-all">
                  ...
                  <div className="flex-1">
                      ...
                      <p className={`text-xs font-bold mt-1 ${p.stock <= 2 ? 'text-red-500' : 'text-slate-400'}`}>
                          Estoque: {p.stock !== undefined ? p.stock : 'N/A'}
                      </p>
                  </div>
                 ...
                 <div className="pt-6 border-t border-slate-100">
                    ...
                    <button onClick={async () => {
                      ...
                      await addDoc(collection(db,"orders"),{
                        ...
                        items: manualCart, total: manualCart.reduce((a,i)=>a+(i.price*i.quantity),0), status:'pending', createdAt: serverTimestamp(),
        ...
        {activeTab === 'marketing' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className={`p-12 rounded-[4rem] shadow-2xl transition-all border-4 ${settings.promoActive ? 'bg-orange-500 text-white border-orange-300' : 'bg-white border-transparent'}`}>
                ...
                }} className={`w-full py-8 rounded-[2.5rem] font-black uppercase tracking-widest text-xl shadow-2xl mt-8 ${settings.promoActive ? 'bg-slate-900' : 'bg-orange-600 text-white'}`}>
                ...
                <div className="mt-10 pt-6 border-t border-slate-100 space-y-4">
                    ...
                    <div className="flex flex-col items-center gap-4">
                        {(promoBannerFile || settings.promoBannerUrl) && (
                            <img src={promoBannerFile ? URL.createObjectURL(promoBannerFile) : settings.promoBannerUrl} className="w-full max-w-lg h-40 object-cover rounded-2xl bg-slate-50"/>
                        )}
                        <input type="file" accept="image/*" onChange={(e) => setPromoBannerFile(e.target.files[0])} className="hidden" id="promo-banner-upload"/>
                        <label htmlFor="promo-banner-upload" className="w-full max-w-lg p-4 bg-slate-50 rounded-2xl flex items-center justify-center gap-3 font-bold text-slate-600 cursor-pointer border-2 border-dashed border-slate-200">
                             Upload Banner <UploadCloud size={20}/>
                        </label>
                        {promoBannerFile && (
                            <button type="button" onClick={handlePromoBannerUpload} disabled={uploadingPromoBanner} className="w-full max-w-lg p-3 bg-blue-600 text-white rounded-2xl font-black">
                                {uploadingPromoBanner ? 'Enviando...' : 'Confirmar Upload'}
                            </button>
                        )}
                    </div>
                </div>
            ...
            <motion.div initial={{scale:0.9, y:20}} animate={{scale:1, y:0}} className="bg-white w-full max-w-xl rounded-[3.5rem] p-12 shadow-2xl relative">
              ...
              <form onSubmit={async (e) => {
                ...
                <div className="space-y-3">
                    ...
                    {imageFile && (
                        <button type="button" onClick={handleProductImageUpload} disabled={uploading} className="w-full p-4 bg-blue-600 text-white rounded-3xl font-black">
