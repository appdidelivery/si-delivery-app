...
import { Loader2, ExternalLink, Save, CheckCircle, Send, RefreshCw, MessageSquare, Search, Sparkles, UploadCloud, X } from 'lucide-react';
export default function GoogleIntegrationDashboard({ storeId, products, storeStatus, uploadImageToCloudinary }) {
    ...
    const fetchProfileData = async () => {
        ...
        } catch (error) { console.error("Erro ao buscar perfil"); }
    };
    ...
    const handleGenerateAICopy = async () => {
        ...
        } catch (error) { alert("Erro ao gerar IA."); }
    ...
    const handleSyncCatalog = async () => {
        ...
        } catch (error) { alert(`❌ Erro ao sincronizar catálogo: ${error.message}`); } 
        finally { setIsSaving(false); }
    };
