...
const paymentsStr = acceptedList.length > 0 ? acceptedList.join('\n') : 'Consulte as opções de pagamento no fechamento do seu pedido.';
                                            ...
                                            if (needsSupport) {
                                                ...
                                                if (cartData && cartData.savedAddress) {
                                                    const addressText = cartData.savedAddress;
                                                    ...
                                                    try {
                                                        ...
                                                        zoneFound = true; // Força fallback se api falhar
                                                    }
                                                    ...
                                                    if (!replyPayload && zoneFound) {
                                                        ...
                                                        logTextForPanel = `🤖 [Confirmou Endereço Antigo - Frete: R$ ${freteCalculado.toFixed(2)}]`;
                                                    }
                                                }
                                            }
  ...
  // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
        ...
        try {
            ...
            // --- 1. VERIFICAÇÃO DE CACHE (Bypass de IA) ---
            const cacheString = `${lojaNicho}-${termoRaw}`.toLowerCase().trim();
            const cacheKey = crypto.createHash('md5').update(cacheString).digest('hex');
            const cacheRef = db.collection('ai_product_copy_cache').doc(cacheKey);
            
            const cacheSnap = await cacheRef.get();
            if (cacheSnap.exists) {
                const data = cacheSnap.data();
                return res.status(200).json({ success: true, nome: data.nome, descricao: data.descricao });
            }

            const GEMINI_KEY = process.env.GEMINI_API_KEY;
            if (!GEMINI_KEY) return res.status(200).json({ success: false, error: "Chave do Gemini ausente na Vercel." });

            const prompt = `Crie Nome e Descrição para delivery.
Loja: ${lojaNome || 'Delivery'} (${lojaNicho || 'Geral'}) - Região: ${lojaLocalizacao || 'Local'}.
Produto buscado: "${termoRaw}".

REGRAS ESTRITAS:
- Retorne APENAS um JSON válido no formato exato: {"nome": "...", "descricao": "..."}.
- É TERMINANTEMENTE PROIBIDO usar marcadores markdown como \`\`\`json. Sem introduções.
- Descrição MÁXIMA de 200 caracteres, tom apetitoso e humano.`;

            // --- 2. CHAMADA OTIMIZADA PARA GEMINI FLASH (Custo Mínimo) ---
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_KEY}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                    ...
                    generationConfig: {
                        temperature: 0.7,
                        maxOutputTokens: 150, // Trava de Tokens!
                        responseMimeType: "application/json" // Força a saída JSON estruturada
                    }
                })
            });
            ...
            if (!response.ok) {
                ...
                return res.status(200).json({ success: false, error: "Falha na API do Google." });
            }

            const rawJsonText = aiData.candidates?.[0]?.content?.parts?.[0]?.text;
            if (!rawJsonText) return res.status(200).json({ success: false, error: "Resposta textual vazia da IA." });
    ...
    // ------------------------------------------------------------------------
    // 22. GERADOR DE COPY PARA PROMOÇÕES OTIMIZADO (COM CACHE E TRAVA)
    // ------------------------------------------------------------------------
        ...
        try {
            ...
            // --- 1. VERIFICAÇÃO DE CACHE (Evita gasto repetitivo na mesma loja/produto) ---
            ...
            if (cacheSnap.exists) {
                const data = cacheSnap.data();
                return res.status(200).json({ success: true, whatsapp: data.whatsapp, instagram: data.instagram, hashtags: data.hashtags });
            }
            ...
            const prompt = `Crie textos curtos de venda. Loja: ${storeName} (${storeNiche}). Produto: ${productName} (R$ ${Number(productPrice).toFixed(2)}). Link: ${exactProductLink}

REGRAS ESTRITAS:
- Retorne APENAS um JSON válido. É PROIBIDO usar markdown (\`\`\`json).
- O formato DEVE ser exato: {"whatsapp": "1 frase curta persuasiva com o link no final", "instagram": "1 legenda muito curta com CTA pra bio", "hashtags": "#4 hashtags"}
- Proibido enviar chaves adicionais no JSON.`;
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_KEY}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    generationConfig: {
                        temperature: 0.7,
                        maxOutputTokens: 250, // Trava de Tokens de Segurança!
                        responseMimeType: "application/json" // Força saída limpa
                    }
                })
            });
    ...
    // ------------------------------------------------------------------------
        ...
        try {
           ...
           // PASSO 2: Criar o Conjunto de Anúncios (Apenas Facebook para isolar erros)
            ...
            // 🚨 PASSO 3: UPLOAD DA IMAGEM PARA A BIBLIOTECA DA META
            ...
            let imageHash = null;
            try {
                // Manda a Meta buscar a URL e guardar na sua biblioteca de anúncios
                ...
                if (imgData.images && Object.keys(imgData.images).length > 0) {
                    ...
                    imageHash = imgData.images[firstKey].hash; // 👈 O Passaporte VIP!
                } else if (imgData.error) {
                    console.error("A Meta rejeitou a imagem:", imgData.error.message);
                }
            } catch (err) {
                console.error("Falha ao subir imagem para a Meta:", err);
            }

            // Se não gerou Hash, não tentamos forçar. Avisamos o lojista.
            if (!imageHash) {
                return res.status(400).json({ error: "A Meta não conseguiu baixar a foto deste produto. Verifique se a foto é válida ou tente anunciar outro item." });
            }

            // Ajuste de Link Localhost
            ...
            // PASSO 4: Criar o Criativo usando a HASH NATIVA (Sem falhas!)
            ...
            // PASSO 5: Criar o Anúncio Final (Liga o Criativo ao Conjunto)
            const ad = await fetchMeta(`${adAccountId}/ads`, {
                name: `Anúncio Velo - ${productName}`,
                adset_id: adSet.id,
                creative: { creative_id: creative.id },
                status: 'PAUSED' // Vai pausado por segurança
            });
...
// ------------------------------------------------------------------------
   ...
   // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // 26.3.1 GOOGLE MEU NEGÓCIO: SINCRONIZAR HORÁRIOS REGULARES
    // ------------------------------------------------------------------------
    else if (path === '/api/sync-google-regular-hours') {
        if (req.method !== 'POST') return res.status(405).json({ error: 'Método não permitido.' });
        try {
            const { storeId, locationId, schedule } = req.body;
            if (!storeId || !locationId || !schedule) return res.status(400).json({ error: 'Dados incompletos.' });

            const activeToken = await getGoogleAuthToken(storeId);
            const stdLocation = `locations/${locationId.split('/').pop()}`;

            // Mapeamento Velo (0 a 6) para Google (SUNDAY a SATURDAY)
            const daysMap = { 0: 'SUNDAY', 1: 'MONDAY', 2: 'TUESDAY', 3: 'WEDNESDAY', 4: 'THURSDAY', 5: 'FRIDAY', 6: 'SATURDAY' };
            const periods = [];

            // Função para converter "08:30" em { hours: 8, minutes: 30 }
            const parseTime = (timeStr) => {
                if (!timeStr) return null;
                const [h, m] = timeStr.split(':').map(Number);
            ...
            for (let i = 0; i <= 6; i++) {
                const dayConfig = schedule[i];
                if (dayConfig && dayConfig.open) {
                    // Adiciona o Primeiro Turno
                    if (dayConfig.start && dayConfig.end) {
                        periods.push({
                            openDay: daysMap[i],
                            openTime: parseTime(dayConfig.start),
                            closeDay: daysMap[i],
                            closeTime: parseTime(dayConfig.end)
                        });
                    }
                    // Adiciona o Segundo Turno (Almoço/Janta), se houver
                    if (dayConfig.splitShift && dayConfig.start2 && dayConfig.end2) {
                        periods.push({
                            openDay: daysMap[i],
                            openTime: parseTime(dayConfig.start2),
                            closeDay: daysMap[i],
                            closeTime: parseTime(dayConfig.end2)
                        });
                    }
                }
            }

            const googlePayload = { regularHours: { periods } };

            const resGoogle = await fetch(`https://mybusinessbusinessinformation.googleapis.com/v1/${stdLocation}?updateMask=regularHours`, {
                method: 'PATCH',
                headers: { 'Authorization': `Bearer ${activeToken}`, 'Content-Type': 'application/json' },
                body: JSON.stringify(googlePayload)
            });
            ...
            const data = await resGoogle.json();
            if (!resGoogle.ok) throw new Error(data.error?.message || 'Falha ao enviar horários ao Google.');

            return res.status(200).json({ success: true });
        } catch (error) {
            console.error('Erro ao sincronizar Horários Regulares:', error);
            return res.status(500).json({ error: error.message });
        }
    }
          ...
          // AÇÃO 3: GERENCIAR INSTÂNCIA E GERAR QR CODE (EVOLUTION)
            if (action === 'evo_manager') {
                ...
                try {
                    ...
                    console.error("[EVO LOG FATAL] Erro de rede:", err);
                    return res.status(500).json({ error: `Falha de rede ao contatar a VPS: ${err.message}` });
                }
            }
