...
// ============================================================================
// MÓDULO DE PROSPECÇÃO CRM: DISPARO WHATSAPP CLOUD API (META)
// ============================================================================
exports.triggerWhatsAppProspect = functions.firestore
    .document('leads_prospeccao/{leadId}')
    .onUpdate(async (change, context) => {
        const before = change.before.data();
        const after = change.after.data();

        // Regra de Ouro: Só dispara se o status mudou EXATAMENTE para 'pending_trigger'
        if (after.wppCloudStatus === 'pending_trigger' && before.wppCloudStatus !== 'pending_trigger') {
            
            // ATENÇÃO LOJISTA: Configure estas variáveis no ambiente do Firebase ou substitua pelas suas Strings
            // Exemplo de como setar no terminal: firebase functions:config:set meta.token="SEU_TOKEN" meta.phone_id="SEU_ID"
            const META_TOKEN = process.env.META_API_TOKEN || "COLOQUE_SEU_TOKEN_AQUI"; 
            const PHONE_ID = process.env.META_PHONE_ID || "COLOQUE_SEU_PHONE_ID_AQUI";
            
            // O nome exato do Template aprovado lá no painel da Meta
            const TEMPLATE_NAME = "abordagem_inicial_crm"; 

            try {
                // Formatação do Telefone (Garante DDI 55)
                let cleanPhone = String(after.phone).replace(/\D/g, '');
                if (cleanPhone.length >= 10 && !cleanPhone.startsWith('55')) {
                    cleanPhone = `55${cleanPhone}`;
                }

                // Payload Oficial exigido pela Meta Cloud API para iniciar conversas
                const payload = {
                    messaging_product: "whatsapp",
                    recipient_type: "individual",
                    to: cleanPhone,
                ...
                // Disparo HTTP nativo do Node.js 18+
                ...
                if (response.ok) {
                    // Sucesso: Atualiza o lead com o ID da mensagem oficial da Meta
                    await change.after.ref.update({
                        wppCloudStatus: 'sent',
                        wppMessageId: data.messages[0].id,
                        status: 'contacted' // Confirma que ele está na coluna de abordados
                    });
                    console.log(`✅ [CRM] Template ${TEMPLATE_NAME} enviado para ${cleanPhone}`);
                } else {
                    throw new Error(data.error?.message || JSON.stringify(data));
                }

            } catch (error) {
                console.error("❌ [CRM] Erro no disparo da Meta:", error);
                // Salva o erro no Firestore para você poder auditar no painel depois
                await change.after.ref.update({
                    wppCloudStatus: 'error',
                    wppError: error.message
                });
            }
        }
        return null;
